...
show_menu() {
    ...
    echo "------------------------------------------------------------"
    echo -e " ${YELLOW}[CLIENTS & SUBS]${NC}        ${YELLOW}[UPDATES & SSL]${NC}        ${YELLOW}[SYSTEM & LOGS]${NC}"
    echo " 1) Add Client          5) Update Uncut Core   9) System Health"
    echo " 2) Edit Client         6) Update Sing-box     10) Restart Services"
    echo " 3) Remove Client       7) Renew SSL Cert      11) Service Logs"
    echo " 4) List Clients & QR   8) Change Domain       12) MTProto (Telegram)"
    echo "                                               13) Uninstall"
    echo ""
    echo " 0) Exit"
    echo "------------------------------------------------------------"
